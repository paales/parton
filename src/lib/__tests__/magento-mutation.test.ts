...
describe("Magento mutations (cart flow)", { timeout: 30000 }, () => {
	it("creates an empty cart", async () => {
		const data = await executeMutation<{ createEmptyCart: string }>(
	...
	it("cart lifecycle: create → add → query", async () => {
		// Step 1: Create cart
		const { createEmptyCart: cartId } = await executeMutation<{ createEmptyCart: string }>(
			`mutation { createEmptyCart }`,
		);
		...
		const addResult = await executeMutation<{
			addProductsToCart: { cart: { items: Array<{ quantity: number; product: { sku: string } }> } };
		}>(`
			mutation {
				addProductsToCart(
					cartId: "${cartId}"
