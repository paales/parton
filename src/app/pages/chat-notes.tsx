 ...
 * UI lives in `<ChatOverlay>`, mounted on every page via the shared
 * layout. See `notes/STREAMING_CHAT.md` for the long-form write-up.
 *
 ...
 * on initial render. The layout reads this via `chatOverlayFrameUrl()`.
 */
export function chatOverlayFrameUrl(): string {
  ...
  return (
    <main className="pb-[60vh]">
      ...
      <h1 className="mb-4 text-2xl font-semibold">
        Chat — streaming the <code className="rounded bg-muted px-1.5 py-0.5 text-[0.85em]">notes/</code> directory
      </h1>
      <p className="mb-3 leading-relaxed text-muted-foreground">
        The box in the bottom-right streams one markdown file from{" "}
        <code className="rounded bg-muted px-1.5 py-0.5 text-[0.85em]">notes/</code>, character by character, through a bounded
        recursive <code className="rounded bg-muted px-1.5 py-0.5 text-[0.85em]">&lt;Piece&gt;</code> server component. When the
        Piece chain hits its depth bound it compacts via a targeted{" "}
        <code className="rounded bg-muted px-1.5 py-0.5 text-[0.85em]">reload</code> — the message re-renders as a synchronous flat
      ...
      <p className="mb-3 leading-relaxed text-muted-foreground">
        The server-side log (<code className="rounded bg-muted px-1.5 py-0.5 text-[0.85em]">src/app/chat/log.ts</code>) holds all
        ...
        <code className="rounded bg-muted px-1.5 py-0.5 text-[0.85em]">?cursor-&lt;fileId&gt;=N</code>) so page reloads and browser
