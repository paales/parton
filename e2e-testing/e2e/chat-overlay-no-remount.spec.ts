...
test("opening the chat overlay does not remount the page while notes stream", async ({
  ...
  try {
    ...
    // The heartbeat opens a chat-closed `?streaming=1` connection on load.
    // Track when it closes (idle keepalive) — only after it reopens does it
    // render the now-open chat and overlap the frame long-poll.
    let streamingClosed = false
    page.on("requestfinished", (req) => {
      const u = req.url()
      if (u.includes("_.rsc") && new URL(u).searchParams.get("streaming") === "1") {
