...
test("typing in the frame search navigates the frame, not the page", async ({ page }) => {
  await page.goto("/")
  await awaitHydrated(page)
  const beforeUrl = page.url()
...
test("search input keeps focus across live refetches", async ({ page }) => {
  ...
  const stillStamped = await page
    ...
    .evaluate((el) => (el as HTMLInputElement & { __stamp?: number }).__stamp === 42)
  expect(stillStamped, "input DOM node was remounted by reconciliation").toBe(true)
