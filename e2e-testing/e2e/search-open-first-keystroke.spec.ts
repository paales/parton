...
async function startKeystroke(page: import("@playwright/test").Page, n: number) {
...
async function readStates(page: import("@playwright/test").Page, includeFinal = true) {
...
test("URL mode: first keystroke streams stages 2/3 progressively", async ({ page }) => {
  const errors: string[] = []
  ...
  expect(k1SawS2Fallback, "URL mode: stage-2 fallback should appear during keystroke 1").toBe(true)
  expect(k1SawS3Fallback, "URL mode: stage-3 fallback should appear during keystroke 1").toBe(true)
  expect(k1.finalS2, "URL mode: stage-2 content should stream in after keystroke 1").toBe(true)
  expect(k1.finalS3, "URL mode: stage-3 content should stream in after keystroke 1").toBe(true)
