...
import { expires, localCell, parton, time, type RenderArgs } from "@parton/framework"
import { LaneSlowBumpButton, SlowCommitBeacon } from "../components/lanes-demo-button.tsx"
    ...
    return (
      <main className="mx-auto flex max-w-2xl flex-col gap-6 p-8">
        ...
        <p className="text-sm opacity-70">
          The clock ticks every second over its own lane. Bumping the slow counter opens a ~2.5s
          lane — and the clock keeps ticking through it, because lanes interleave instead of gating
          on the slowest render. After it lands, the counter reports the server-second window it
          rendered through: the clock will have displayed those very seconds while the render was
          still in flight.
        </p>
        <LaneClock />
        <LaneSlowCounter />
