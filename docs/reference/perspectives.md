...
2. **Fingerprint-based skip**: Like cache-keying every region by its actual dependencies: each partial hashes `(spec id, match params, tracked-read dep records, cell partitions, CMS contribution, call-site props, invalidation stamps, descendant fold)` into a fingerprint. The client sends its fp set on every refetch; the server skips body execution when its current fp matches.
3. **Define-step constructor**: Like `React.memo`, but at module scope and request-shaped: `parton(Render, options)` runs once at import time and returns a placeable component whose request dependencies are the tracked reads its body actually performs — the read IS the dependency, recorded per render and folded into the fingerprint. Page composition is a JSX tree of those components.
...
9. **Livewire**: Like Livewire-style server actions for RSC fragments: a server action calls `getServerNavigation().reload({selector: "#cart"})` in its body and the framework re-renders every partial matching that selector on the action response (transactionally — a throw discards the bump).
