...
interface PartialOptions {
  ...
  cache?: CacheOptions
  defer?: true | "stream" | ReactElement<ActivatorProps>
  fallback?: ReactNode
...
| `defer`          | `true` for app-driven, an activator element to wire automatically, or `"stream"`: the parton's body always defers OUT of enclosing DRIVER-OWNED bodies (connection lane renders, covering nav/reconcile segments) — the enclosing body ships a pending-marked placeholder and closes at its shell, and the driver immediately delivers this parton's full body on its own forced follow-up lane, no client trigger. Declare it on a slow streamed member (a live price, a review feed) so a parent's materialization never waits for it — the parent's shell commits with this parton's region suspended (the ambient `Suspense` fallback holds) and the content lands moments later. Renders with no driver (documents, SSR, discrete refetches) render it deep — same output, different pacing. |
