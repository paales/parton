...
| `publish`    | Optional `boolean \| (capability) => boolean`. Publishes the cell ACROSS THE PROCESS BOUNDARY: another parton app may attach to its committed bumps and read its value through this app's `/__remote/cells/*` endpoints (the `remoteCell` contract — [`remote-frame.md`](./remote-frame.md#remotecell--outward-state)). A callback authorizes each caller's presented capability bag. Default: never served (403). |
