...
under the type derived from the Render name (`PromoRender` →
`"promo"`), which is also the storage key a singleton placement reads
(see [`block.md`](./block.md#identity--the-render-functions-name)) —
rename the Render to move it. Import the file once for its side
effect (`e2e-testing/src/app/blocks/catalog.ts` does this for the
demo app).

## Slots

Slot composition lives on the schema. `cms.blocks(slot)`
returns a ReactNode that resolves every entry under `node.slots[slot]`
to a rendered block via the type catalog; `cms.block(slot)`
...
- Tree — registry-driven view of the CMS content rows that rendered
  for the previewed page. The tree reads `getRouteSnapshots()` (every
  CMS-bound block self-registers at render time), keeps the snapshots
  whose spec is registered as a slot block via
  `getSlotBlockMeta(snap.type)`, and collects their `cms:` deps as
  the row keys — a render id is a placement-scoped effective id
  (props hash, placement fold), not a row key, so the dep set is
  where the row is named explicitly. Each collected key walks as a
  ...
  so cross-page navigation invalidates the tree fp, and the
  `cms-edit-tree` tag, so a structural write's
  `refreshSelector("cms-edit-tree")` re-renders it.
...
- Field form — per-config tabs + form fields derived from the catalog
  manifest. Reads `pathname()` so `pickBestConfigIndex` re-evaluates
  as the previewed page changes, and the `cms-edit-fields` tag, so a
  field write's `refreshSelector("cms-edit-fields")` re-renders it.

Both chrome partons set `fpSkip: false` — always authoritative, never
served from the client's fp cache, because their links embed the full
URL.

Server actions (`saveCmsFields`, `publishCmsDraft`,
`addBlockToSlot`, `removeBlockFromSlot`, `moveBlockInSlot`,
`resetCmsDraft`) live in `cms/src/editor/actions.ts`. A field write
fires `refreshSelector(["cms:<id>", "cms-edit-tree",
"cms-edit-fields"])` — the edited row's tag wakes the preview's bound
block instances, the two chrome tags wake the panels.
