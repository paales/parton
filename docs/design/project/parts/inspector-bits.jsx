...
function SelectionRect({
  ...
  const stroke = variant === 'amber' ? '#d99a2c'
                : variant === 'pink'  ? '#c75a7a'
                : 'var(--insp-blue-strong)';
  return (
    <div style={{ position:'relative', width, height, ...style }}>
      {/* margin (outermost stripes) */}
      {showMargin && (
        <div className="stripes-margin"
             style={{ position:'absolute', inset: 0, borderRadius: 0 }} />
      )}
      {/* padding (inset) */}
      {showPadding && (
        <div className="stripes-padding"
...
function StripeZone({ children, kind = 'margin', style, label }) {
  const cls = kind === 'padding' ? 'stripes-padding'
            : kind === 'content' ? 'stripes-content'
            : kind === 'gray'    ? 'stripes-gray'
            : 'stripes-margin';
  return (
    <div className={cls} style={{
      ...
      {label && <div className="mono" style={{ fontSize:10, color:'var(--ink-2)', background:'#fff', padding:'2px 6px', borderRadius:3 }}>{label}</div>}
      {children}
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {sub && <div className="mono" style={{ fontSize: 10, color:'var(--ink-3)' }}>{sub}</div>}
        <div style={{ flex: 1 }} />
        {onClose !== false && (
          <div style={{ width: 18, height: 18, display:'grid', placeItems:'center',
                       borderRadius: 4, color:'var(--ink-2)', cursor:'pointer' }}>×</div>
        )}
      </div>
      ...
      {footer && (
        <div style={{
          height: 30, padding: '0 10px', borderTop:'1px solid rgba(0,0,0,0.08)',
          display:'flex', alignItems:'center', gap:8, fontSize: 11, color:'var(--ink-2)',
...
function Row({ label, children, hint }) {
  return (
    <div style={{ display:'grid', gridTemplateColumns:'88px 1fr', gap: 10, alignItems:'center', marginBottom: 10 }}>
      <div style={{ fontSize: 12, color:'var(--ink-2)', fontFamily:'var(--ui)', fontWeight: 400 }}>{label}</div>
      <div>{children}{hint && <div className="mono" style={{ fontSize: 12, color:'var(--ink-3)', marginTop: 3 }}>{hint}</div>}</div>
    </div>
  );
}
...
function SectionHead({ children }) {
  return (
    <div style={{
      fontSize: 11,
      color:'var(--ink-3)', fontFamily:'var(--ui)', fontWeight: 500,
...
function Glyph({ kind = 'block', size = 12, color = 'var(--ink-2)' }) {
  const s = size;
  if (kind === 'plus') return <div style={{ width:s, height:s, position:'relative' }}>
    <div style={{ position:'absolute', left:0, right:0, top:'50%', height:1.5, background:color, transform:'translateY(-50%)' }} />
    <div style={{ position:'absolute', top:0, bottom:0, left:'50%', width:1.5, background:color, transform:'translateX(-50%)' }} />
  </div>;
  if (kind === 'chev') return <div style={{
    width:s*0.5, height:s*0.5,
    borderRight: `1.5px solid ${color}`, borderBottom: `1.5px solid ${color}`,
    transform:'rotate(-45deg)',
  }} />;
  if (kind === 'caret') return <div style={{
    width: 0, height: 0,
    borderTop: `${s*0.45}px solid ${color}`,
    borderLeft: `${s*0.45}px solid transparent`,
    borderRight: `${s*0.45}px solid transparent`,
  }} />;
  if (kind === 'grip') return <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1.5, width:s, height:s }}>
    {[0,1,2,3,4,5].map(i => <div key={i} style={{ width: 2, height: 2, background: color, borderRadius: '50%' }} />)}
  </div>;
