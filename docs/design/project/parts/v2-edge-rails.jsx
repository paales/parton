  ...
  return (
    <div style={{ position:'relative', width: W, height: H, background:'#e9e4d8' }}>
      ...
      <div style={{ position:'absolute', left: 30, top: 480, pointerEvents:'none' }}>
        ...
        <div className="stripes-margin" style={{ position:'absolute', left: 0, right:0, top: -8, height: 8 }} />
        <div className="stripes-margin" style={{ position:'absolute', left: 0, right:0, bottom: -8, height: 8 }} />
        {/* corners */}
        {[
          { left: -4, top: -4 }, { right: -4, top: -4 },
          { left: -4, bottom: -4 }, { right: -4, bottom: -4 },
        ].map((p,i) => <div key={i} className="corner" style={p} />)}
      </div>

      {/* ─── Left rail (collapsed layers) ─── */}
      <div style={{
        position:'absolute', left: 8, top: 86, bottom: 12,
        width: 44,
      ...
      <div style={{
        position:'absolute', left: 60, top: 86,
        width: 220, height: 360,
        background:'rgba(255,255,255,0.96)',
        border:'1px solid rgba(0,0,0,0.12)',
        borderRadius: 10,
        boxShadow:'0 12px 30px rgba(0,0,0,0.12)',
        padding: 10, fontFamily:'var(--ui)',
      }}>
        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom: 8 }}>
          <Glyph kind="caret" /><div style={{ fontSize: 12, fontWeight: 600 }}>Home page</div>
        </div>
        <div className="wf-field" style={{ marginBottom: 8 }}>Search blocks…</div>
        {[
          { d:0, l:'Header', g:true },
          { d:0, l:'Hero', g:true },
          ...
          <div key={i} style={{
            ...
            padding:'5px 6px', paddingLeft: 6 + r.d * 14,
      ...
      <div style={{
        position:'absolute', right: 8, top: 86, bottom: 12,
        width: 280,
        background:'rgba(255,255,255,0.96)',
        border:'1px solid rgba(0,0,0,0.12)',
        borderRadius: 10,
        boxShadow:'0 12px 30px rgba(0,0,0,0.12)',
        padding: 12, fontFamily:'var(--ui)', fontSize: 12,
        overflow:'hidden',
      }}>
        ...
        <div style={{ marginTop: 10 }}>
          ...
          <Row label="Source"><div className="wf-field">Red wines</div></Row>
          <Row label="Type"><div className="wf-segment"><div className="on">Carousel</div><div>Grid</div></div></Row>

          <SectionHead>Layout</SectionHead>
          <Row label="Products" hint="8"><div className="wf-slider" /></Row>
          <Row label="Columns" hint="4"><div className="wf-slider" /></Row>
          <Row label="Gap" hint="16 px"><div className="wf-slider" /></Row>
          <Row label="Width"><div className="wf-segment"><div>Page</div><div className="on">Full</div></div></Row>
          <Row label="Align"><div className="wf-segment"><div>L</div><div className="on">C</div><div>R</div></div></Row>

          <SectionHead>Spacing</SectionHead>
          {/* spacing visualizer with stripe vocabulary */}
          <div style={{ position:'relative', height: 90, background:'#fff', border:'1px solid rgba(0,0,0,0.1)', borderRadius: 6 }}>
            <div className="stripes-margin" style={{ position:'absolute', inset: 6, borderRadius: 4 }} />
            <div className="stripes-padding" style={{ position:'absolute', inset: 18, borderRadius: 4 }} />
            <div style={{ position:'absolute', inset: 30, background:'rgba(108,177,240,0.18)', outline:'1px dashed var(--insp-blue-strong)' }} />
            ...
            {[
              ...
              <div key={i} className="mono" style={{
                position:'absolute', ...p, fontSize: 10, color:'var(--ink-2)',
              }}>{p.t}</div>
            ))}
          </div>
        </div>
      ...
      <Annotation x={70} y={H - 50} text="rails: collapsed by default · layers fly out · property drawer is the only persistent panel" w={520} />
    </div>
  );
}
