  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        padding: '0 36px', gap: 24, zIndex: 1,
        fontFamily: 'var(--ui)',
      }}>
        <div style={{
          width: 110, height: 14, background: '#1a1a1f', borderRadius: 2,
        }} />
        <div style={{ flex: 1, display: 'flex', gap: 22 }}>
          {['Shop', 'Categories', 'Sale', 'About', 'Contact'].map(x => (
            <div key={x} style={{ height: 10, width: 50 + x.length * 3, background: 'rgba(0,0,0,0.18)', borderRadius: 2 }} />
          ))}
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          {[0,1,2].map(i => (
            <div key={i} style={{ width: 18, height: 18, borderRadius: '50%', background: 'rgba(0,0,0,0.12)' }} />
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          marginTop: 16, width: 130, height: 38, background: '#1a1a1f',
        ...
        {[0,1,2].map(i => (
          <div key={i} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8 }}>
            <div style={{
              ...
              fontFamily: 'var(--mono)', fontSize: 10, color: 'rgba(0,0,0,0.55)',
              background: 'rgba(0,0,0,0.03)',
            }}>icon</div>
            <div style={{ width: 100, height: 9, background: '#1a1a1f', borderRadius: 2 }} />
            <div style={{ width: 160, height: 7, background: 'rgba(0,0,0,0.3)', borderRadius: 2 }} />
          </div>
        ))}
      </div>
      ...
      <div style={{
        position: 'absolute', top: 490, left: 30, right: 30, height: 200,
        boxSizing: 'border-box',
        background: '#fff', border: '1px solid rgba(0,0,0,0.07)',
        display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, padding: 16,
      }}>
        {[0,1,2,3].map(i => (
          <div key={i} style={{ background: '#efeae0', borderRadius: 4, position:'relative' }}>
            <div style={{ position:'absolute', bottom: 12, left: 12, right: 12, height: 8, background: 'rgba(0,0,0,0.45)', borderRadius:2 }} />
            <div style={{ position:'absolute', bottom: 28, left: 12, width: 60, height: 6, background: 'rgba(0,0,0,0.25)', borderRadius:2 }} />
          </div>
        ))}
      </div>
    </div>
