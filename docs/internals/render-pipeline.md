...
5. **Fingerprint + skip decision** (see below and _Fingerprint
   protocol_).
...
- the client's cached manifest contains the computed fp — the attach
  ...
  unattached action POST's capped `x-parton-cached` header), backed by the
...
`null` and the whole row drops — the live Navigation API is the source
of truth on a refetch, so serializing the URL would be dead payload weight.
...
shapes there keep the partials _inside_ the page (e.g. the nav)
reconciling too.
...
| `segmented-response.ts` / `segment-relevance.ts` / `parton-mux.ts` | The server segment driver + lane pump; the route wake subscription (registration into the inverted wake index, sync diff, lane-carrier resolution, the parity oracle); and the per-parton lane mux/demux framing.                                                                                                                                                                                                                                                                                                |
...
| Module                                           | Owns                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `partial-client-state.ts`                        | ALL module-level mutable state, behind accessors: the partial cache + fingerprint maps (`cacheStore`, `registerClientPartial`, `touchClientPartial`, `_applyFpUpdates`, `pruneToLive`; the cached manifest has two forms — the attach statement's BODY manifest (`getAllCachedPartialTokens`) carries everything, no header budget to protect, while the unattached action POST's `x-parton-cached` header form (`getCachedPartialIds`) is doubly bounded: `FP_CAP_PER_VARIANT` fps per variant, `CACHED_MANIFEST_CAP` entries total, parked-by-culling ids first (`_setManifestPriorityIds`) then newest-sighting-first, since it counts against the response's total-header budget and a many-parton page would otherwise blow Node's default 16 KB `maxHeaderSize`; `CLIENT_POOL_CAP` bounds distinct ids, aging out the least-recently-sighted — every walk touches the ids it sees, so recency means "still emitted by commits" — with the displayed tree exempt: `_liveTreeIds` from the payload prune plus lane-commit folds via `_addLiveTreeIds`), the id-pruned listener (`_setIdPrunedListener` — the page-membership teardown signal cull-park registers on), the content-loss listener (`_setContentLossListener` — every destruction of committed content reports its id, and the channel transport rides the ids on the next ack's `evicted` statement), the persisted template (`getTemplate` / `setTemplate`), the lane-commit subscription (`subscribeLaneCommits`, with two notify forms: `notifyLaneCommit` immediate — interactive commits, cancels any pending coalesced flush — and `notifyLaneCommitCoalesced`, the lane flush quantum: streaming lane traffic coalesces to ONE subscriber notify per animation frame, so the template re-render + React commit rate is bounded by the paint rate; a timer backstop races the frame callback since rAF alignment is an optimization, not a liveness signal — a page with no frame flow (hidden, occluded background tab) would otherwise never render lane content. Interactive lanes and first fills are exempt — see `_commitPartonLane`), the live-connection id (`_setLiveConnectionId` / `_getLiveConnectionId`) + the take-once catch-up anchor, and the frame-URL cache. |
...
| `channel-client.ts`                              | The channel's client transport ([`channel.md`](./channel.md)): envelope assembly + per-connection seq, rAF-coalesced serialized flushes over registered producers (the establishment ack, the first-delivery ack, and the threshold-crossing ack are the three flush drivers — §The `ack` frame), the delivery-failure hand-back, connection lifecycle (`_channelWireEntry` establishment from the stream's `conn` handshake, the presence-only `data-parton-live` marker), and the `pagehide` detach frame. **Not in the hydration bundle** — loaded lazily by `live-boot.tsx` after first commit; producers speak to it through `channel-registry.ts` before it binds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `live-boot.tsx`                                  | The lazy live layer's installer. `bootBrowser` hydrates lean (no channel/heartbeat in the entry chunk); a post-commit effect imports this module, which binds the channel transport upstream (`_bindChannelUpstream` — replaying any statements the registry queued pre-bind) and installs the heartbeat imperatively. The live/channel/refetch code is thus a separate lazy chunk off the first-paint critical path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `channel-registry.ts`                            | The channel's eager producer-facing seam (in the hydration closure, imports nothing heavy). Producers (visibility, telemetry, content-loss) register HERE, not against the transport; pre-bind statements (`scheduleChannelFlush`, `_reportContentEvicted`) latch and replay in order when `live-boot` binds the transport (`_bindChannelUpstream`). One-way dependency: the transport imports the registry, never the reverse — so a producer's static closure stays free of the transport.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `frame-context.tsx`                              | The eager half of the frames layer (contexts, providers, path helpers, the frames-tree model) — split out of `frame-client.tsx` so the frame contexts hydrate without dragging the channel-touching frame handles into the entry chunk.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `live-page-heartbeat.tsx`                        | The held connection's keeper (installed imperatively by `live-boot.tsx` after first commit, not mounted in the hydration tree) — each fire is the ATTACH: a `POST /__parton/live` whose JSON body is the full client statement, carrying the measured visible seed (the first fire waits for the first viewport measurement when cullable observers are mounted), the uncapped manifest, and the take-once catch-up anchor from the document ([`channel.md`](./channel.md) §The attach). Its ~5s interval tick is the reattach loop — a no-op while a connection is open, the recovery path after a transient tear. Connection ids are server-minted; establishment rides the stream's `conn` entry, not this component (see [`streaming.md`](./streaming.md)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `visibility.tsx` / `page-interactive.ts`         | The viewport observer for cullable partons + the flip controller — the channel's first producer (display-state priming, delta-only dispatch, first-measurement sync, `visible` frames onto the open live connection; with no connection the flips PEND and ride the next attach's first flush). Native observers are POOLED: one `IntersectionObserver` per rootMargin config shared by every slot (per-instance observers pay a native intersection pass per instance per dirtied frame at density) — each slot hands `observeUsing` a facade that subscribes exactly its own elements on the shared instance and routes each native batch back per slot; an element observed by two slots (an outer transparent cullable sharing host children with a nested one) is cycled on the second subscribe so its initial measurement re-fires. Also the `data-parton-interactive` root marker.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
— a LEAN hydration: the Navigation API intercept, the server-action
callback, and the frame contexts, but NOT the channel/heartbeat. A
post-commit effect imports `live-boot.tsx`, which installs the live
layer off the first-paint critical path — the attach transport
(`window.__rsc_live_attach` — the channel's opening statement and the
page's one held stream), the channel transport bound to the eager
`channel-registry.ts` seam, and the heartbeat. Statements a producer
makes before the transport binds (a visibility flip, a content-loss
report) latch in the registry and replay on binding, so nothing is
lost in the hydration→live-layer gap. The lean entry chunk is why
first paint carries ~9 KB gz less than the pre-split boot.
...
| Param                        | Carries                                                                                                                     |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `partials=<id>`              | focused embed-refetch target on an embed-flagged page GET                                                                   |
...
carries it as the `x-parton-cached` header (`id:matchKey:fp,…`, capped;
self-stripping since `match` drops all `x-parton-*`), and an attached
connection supplies it from the session mirror. The attach statement's
`cached` field is the uncapped body form.

`partials`, `__frame` and `__frameUrl` are the `TRANSPORT_PARAMS`
...
- **The parked budget** (`CULL_PARK_CAP`, 64). Culled ids form an
  ...
  walk (`_setManifestPriorityIds` — the unattached action POST's
  `x-parton-cached` header; the attach's body manifest is uncapped): on a busy live page,
...
field carries every token, an unattached action POST serializes the
capped `x-parton-cached` header, and on a held connection the driver appends
...
- every previously-registered descendant spec's contribution — its
  stored `varyKey` + dep record, re-evaluated against the _current_
...
- **RSC response (the attach's held stream _and_ action POSTs)**:
...
The per-partial `<PartialErrorBoundary>` wraps the _resolved body_
...
throw from the component that _renders_ it. Uncaught, such a throw
...
that streams _after_ `renderSpec` returns is still caught by the inner
...
wrapped around `payload.root` _inside_ the host's browser root, so the
...
`<GlobalErrorBoundary>`. It can't rescue a _deferred_ part whose server
