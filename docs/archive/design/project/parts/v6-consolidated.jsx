...
const PALETTES = {
  inspector: { // Chrome devtools — purple/pink + brown attrs + blue values
    SEL: '#881280', SEL_STRONG: '#6e0e68',
    SEL_FILL: 'rgba(136,18,128,0.08)', SEL_FILL_STRONG: 'rgba(136,18,128,0.15)',
    C_TAG: '#881280', C_ATTR: '#994500', C_ATTR_VAL: '#1a1aa6',
    dark: false
  },
  dark: { // Chrome devtools dark mode
    SEL: '#d3a4ff', SEL_STRONG: '#e6c2ff',
    SEL_FILL: 'rgba(211,164,255,0.10)', SEL_FILL_STRONG: 'rgba(211,164,255,0.18)',
    C_TAG: '#d3a4ff', C_ATTR: '#f4b66a', C_ATTR_VAL: '#9bb6ff',
    dark: true
  }
};
function usePalette() {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{
        ...
        height: 22, padding: '0 5px 0 6px',
        background: fieldBg, border: fieldBorder, borderRadius: 5,
        fontFamily: 'var(--ui)', fontSize: T.ui, color: ink, minWidth: 44
      }}>
...
function camelToSpace(s) {
  if (!s) return s;
  return s.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/^./, c => c.toUpperCase()).toLowerCase().replace(/^./, c => c.toUpperCase());
}
...
function usePanelSurface() {
  ...
  if (pal.dark) {
    if (surface === 'translucent') return {
      background: 'rgba(28,28,32,0.78)', border: '1px solid rgba(255,255,255,0.10)',
      boxShadow: '0 16px 40px rgba(0,0,0,0.50)', backdropFilter: 'blur(14px) saturate(140%)',
      WebkitBackdropFilter: 'blur(14px) saturate(140%)', color: '#e6e6ea'
    };
    if (surface === 'solid') return {
      background: '#1c1c20', border: '1px solid rgba(255,255,255,0.12)',
      boxShadow: 'none', color: '#e6e6ea'
    };
    return {
      background: '#1c1c20', border: '1px solid rgba(255,255,255,0.08)',
      boxShadow: '0 12px 30px rgba(0,0,0,0.45)', color: '#e6e6ea'
    };
  }
  if (surface === 'translucent') return {
    background: 'linear-gradient(135deg, rgba(255,255,255,0.78) 0%, rgba(248,243,232,0.55) 100%)',
    border: '1px solid rgba(255,255,255,0.95)',
    boxShadow: '0 1px 0 rgba(255,255,255,0.9) inset, 0 -1px 0 rgba(0,0,0,0.04) inset, 0 18px 50px rgba(60,50,30,0.18), 0 4px 12px rgba(60,50,30,0.08)',
...
function ResizeGrip() {
  return (
    <svg width="12" height="12" viewBox="0 0 12 12"
      style={{ position: 'absolute', right: 3, bottom: 3, cursor: 'nwse-resize',
        color: 'rgba(0,0,0,0.32)', pointerEvents: 'none' }}>
...
function Icon({ name, size = 16, color = 'currentColor', strokeWidth = 1.5 }) {
  ...
  const paths = {
    layers: <><path d="M8 1.5L1.5 5 8 8.5 14.5 5 8 1.5z" /><path d="M1.5 8L8 11.5 14.5 8" /><path d="M1.5 11L8 14.5 14.5 11" /></>,
    settings: <><circle cx="8" cy="8" r="2" /><path d="M13.4 9.4l1.1.6-1 1.7-1.2-.4a4.5 4.5 0 01-1.4.8l-.2 1.3H8.3l-.2-1.3a4.5 4.5 0 01-1.4-.8l-1.2.4-1-1.7 1.1-.6a4.5 4.5 0 010-1.6l-1.1-.6 1-1.7 1.2.4a4.5 4.5 0 011.4-.8l.2-1.3h2.4l.2 1.3a4.5 4.5 0 011.4.8l1.2-.4 1 1.7-1.1.6a4.5 4.5 0 010 1.6z" /></>,
    theme: <><circle cx="8" cy="8" r="6" /><path d="M8 2v12M2 8h12" /></>,
    home: <><path d="M2.5 7L8 2.5 13.5 7v6.5h-3v-4h-5v4h-3V7z" /></>,
    section: <><rect x="2" y="3" width="12" height="10" rx="1" /><path d="M2 7h12" /></>,
    ...
    cols: <><rect x="2" y="2" width="3" height="12" /><rect x="6.5" y="2" width="3" height="12" /><rect x="11" y="2" width="3" height="12" /></>,
    nav: <><path d="M2 4h12M2 8h12M2 12h12" /></>,
    text: <><path d="M3 4h10M3 8h10M3 12h6" /></>,
    heading: <><path d="M3 3v10M3 8h7M10 3v10" /></>,
    image: <><rect x="2" y="2" width="12" height="12" rx="1" /><circle cx="6" cy="6" r="1.2" /><path d="M2 11l4-3 4 3 4-2" /></>,
    ...
    folder: <><path d="M2 4h4l1.5 2H14v6.5H2V4z" /></>,
    eye: <><path d="M1.5 8s2.5-4.5 6.5-4.5S14.5 8 14.5 8 12 12.5 8 12.5 1.5 8 1.5 8z" /><circle cx="8" cy="8" r="1.8" /></>,
    eyeOff: <><path d="M2 8s2.5-4.5 6-4.5c1.2 0 2.3.4 3.2 1M14 8s-2.5 4.5-6 4.5c-1.2 0-2.3-.4-3.2-1" /><path d="M1.5 1.5l13 13" /></>,
    ...
    undo: <><path d="M3 5l-2 2 2 2" /><path d="M1 7h8a4 4 0 014 4v1" /></>,
    redo: <><path d="M13 5l2 2-2 2" /><path d="M15 7H7a4 4 0 00-4 4v1" /></>,
    play: <><path d="M5 3l8 5-8 5V3z" /></>,
    cart: <><path d="M2 3h2l1.5 8h7L14 5H5" /><circle cx="6.5" cy="13.5" r="1" /><circle cx="11.5" cy="13.5" r="1" /></>,
    tag: <><path d="M2 2h6l6 6-6 6-6-6V2z" /><circle cx="5" cy="5" r="1" /></>,
    page: <><path d="M3 2h6l4 4v8H3V2z" /><path d="M9 2v4h4" /></>,
    pen: <><path d="M11 2l3 3-9 9H2v-3l9-9z" /></>,
    lock: <><rect x="3" y="7" width="10" height="7" rx="1" /><path d="M5 7V5a3 3 0 016 0v2" /></>,
    gift: <><rect x="2" y="6" width="12" height="3" /><rect x="3" y="9" width="10" height="5" /><path d="M8 6v8M5 6S4 4 5.5 3.5 8 6 8 6 9 3 10.5 3.5 11 6 11 6" /></>,
    gridList: <><rect x="2" y="2" width="4" height="4" /><rect x="2" y="9" width="4" height="4" /><path d="M8 4h6M8 11h6" /></>,
    exit: <><path d="M7 3h6v10H7" /><path d="M9 8H2M5 5L2 8l3 3" /></>,
    desktop: <><rect x="2" y="3" width="12" height="8" rx="1" /><path d="M5 14h6" /></>,
    tablet: <><rect x="4" y="2" width="8" height="12" rx="1" /></>,
    mobile: <><rect x="5" y="2" width="6" height="12" rx="1" /></>,
    arrows: <><path d="M5 5L2 8l3 3M11 5l3 3-3 3" /></>,
    coin: <><circle cx="8" cy="8" r="6" /><path d="M9.5 6.5h-2.5a1 1 0 100 2h2a1 1 0 110 2H6.5" /><path d="M8 4.5v1M8 10.5v1" /></>,
    diamond: <><path d="M8 2L14 8 8 14 2 8 8 2z" /></>,
    template: <><rect x="2" y="2" width="12" height="3" /><rect x="2" y="6" width="5" height="8" /><rect x="8" y="6" width="6" height="8" /></>,
    trash: <><path d="M3 4.5h10M6.5 4.5V3a1 1 0 011-1h1a1 1 0 011 1v1.5M4.5 4.5v8.5a1 1 0 001 1h5a1 1 0 001-1V4.5" /></>,
    duplicate: <><rect x="2" y="2" width="9" height="9" rx="1" /><path d="M5 14h8a1 1 0 001-1V6" /></>,
    more: <><circle cx="3.5" cy="8" r="0.8" /><circle cx="8" cy="8" r="0.8" /><circle cx="12.5" cy="8" r="0.8" /></>,
    sectionPolaris: <><rect x="2" y="3" width="12" height="10" rx="1.5" /><path d="M2 6h12M2 10h12" /></>,
    headerPolaris: <><rect x="2" y="3" width="12" height="10" rx="1.5" /><path d="M2 6h12" /><circle cx="4.5" cy="4.5" r="0.4" /><circle cx="6" cy="4.5" r="0.4" /><circle cx="7.5" cy="4.5" r="0.4" /></>,
    footerPolaris: <><rect x="2" y="3" width="12" height="10" rx="1.5" /><path d="M2 10h12" /></>,
    columnsPolaris: <><rect x="2" y="3" width="5" height="10" rx="1" /><rect x="9" y="3" width="5" height="10" rx="1" /></>,
    bannerPolaris: <><rect x="2" y="4" width="12" height="8" rx="1.5" /><path d="M2 9l3-2 2 1.5 3-2.5 4 3" /></>,
    quotePolaris: <><path d="M3 6a2 2 0 012-2v2a1 1 0 00-1 1v3H3V6zM9 6a2 2 0 012-2v2a1 1 0 00-1 1v3H9V6z" /></>,
    database: <><ellipse cx="8" cy="3.5" rx="5" ry="1.8" /><path d="M3 3.5v9c0 1 2.24 1.8 5 1.8s5-.8 5-1.8v-9" /><path d="M3 8c0 1 2.24 1.8 5 1.8s5-.8 5-1.8" /></>,
    // Sun (light mode) — circle + 8 short rays.
    sun: <><circle cx="8" cy="8" r="2.5" /><path d="M8 1.5v2M8 12.5v2M1.5 8h2M12.5 8h2M3.4 3.4l1.4 1.4M11.2 11.2l1.4 1.4M3.4 12.6l1.4-1.4M11.2 4.8l1.4-1.4" /></>,
    ...
    floatPanels: <><rect x="1.5" y="1.5" width="13" height="13" rx="1.5" /><rect x="3.5" y="3.5" width="4" height="9" rx="0.8" /><rect x="9.5" y="3.5" width="3" height="9" rx="0.8" /></>,
    // Docked panels — full-height bars flush to the edges, no margin.
    dockPanels: <><rect x="1.5" y="1.5" width="13" height="13" rx="1.5" /><path d="M5 1.5v13M11 1.5v13" /></>
  };
  return <svg {...common}>{paths[name] || paths.block}</svg>;
}

function V6Consolidated() {
  const { SEL, SEL_STRONG, SEL_FILL, SEL_FILL_STRONG, C_TAG, C_ATTR, C_ATTR_VAL, dark } = usePalette();
  const W = 1440,H = 900;
  ...
  return (
    <div data-attachment={attachment} data-surface={tweaks.surface || 'light'} data-dark={dark || undefined}
      ...
      <div style={{ position: 'absolute', left: 30, top: 490, pointerEvents: 'none' }}>
        <div style={{
          width: W - 60, height: 200,
          outline: `1.5px dashed ${SEL}`,
          outlineOffset: -1, background: SEL_FILL
        }} />
        <div className="stripes-margin" style={{ position: 'absolute', left: 0, right: 0, top: -10, height: 10 }} />
        <div className="stripes-margin" style={{ position: 'absolute', left: 0, right: 0, bottom: -10, height: 10 }} />
        {[
        { left: -4, top: -4 }, { right: -4, top: -4 },
        { left: -4, bottom: -4 }, { right: -4, bottom: -4 }].
        map((p, i) => <div key={i} style={{
          position: 'absolute', width: 7, height: 7,
          background: '#fff', border: `1.5px solid ${SEL_STRONG}`, borderRadius: 1, ...p
        }} />)}

        {/* Attached browser-inspector label — sits flush to bottom-left of selection */}
        <div style={{
          position: 'absolute', left: -1, top: '100%', marginTop: 10,
          background: dark ? '#1c1c20' : '#fff',
          border: `1px solid ${SEL}`,
          borderRadius: 3,
          padding: '3px 7px',
          display: 'flex', alignItems: 'center', gap: 8,
          fontFamily: 'var(--mono)', fontSize: T.label,
      ...
      <div style={{ position: 'absolute', left: '50%', top: 470, transform: 'translateX(-50%)', zIndex: 6,
        display: 'flex', alignItems: 'center', gap: 6 }}>
        <div style={{ width: 320, height: 1, background: '#2c7fd6', opacity: 0.35 }} />
        <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#2c7fd6', color: '#fff',
          display: 'grid', placeItems: 'center', boxShadow: '0 4px 10px rgba(44,127,214,0.4)' }}>
...
function TopBar({ pageOpen, setPageOpen }) {
  ...
  return (
    <div data-topbar style={{
      ...
      fontFamily: 'var(--ui)', fontSize: T.ui,
      ...
      ...(ribbon ? {
        ...
        borderRadius: 0, height: 48, padding: '0 16px',
        background: pal.dark ? '#1c1c20' : '#fff',
        border: 'none', borderBottom: pal.dark ? '1px solid rgba(255,255,255,0.10)' : '1px solid rgba(0,0,0,0.10)',
        boxShadow: 'none',
        width: '100%', justifyContent: docked ? 'center' : 'flex-start'
      } : {
        left: '50%', top: 14, transform: 'translateX(-50%)',
        borderRadius: 10, height: 44, padding: '0 6px',
        ...surface,
      ...
      {!docked && (
        <div style={{ width: 22, height: 28, display: 'grid', placeItems: 'center',
          color: ink3, cursor: 'grab', opacity: 0.6 }} title="Drag toolbar">
          <SixDot />
        </div>
      )}
      <div style={{ width: 28, height: 28, borderRadius: 6,
        display: 'grid', placeItems: 'center', color: ink2, cursor: 'pointer' }} title="Exit design mode">
      ...
      style={{ display: 'flex', alignItems: 'center', gap: 6, height: 30, padding: '0 10px',
        ...
        <span style={{ fontWeight: 400, color: pal.dark ? '#f1f1f4' : 'var(--ink)' }}>Home page</span>
        <Icon name="chevDown" size={ICON} color={pal.dark ? 'rgba(241,241,244,0.6)' : 'var(--ink-3)'} />
      </div>
      <Sep />
      <div style={{ display: 'flex', gap: 2 }}>
        {[
        { i: 'desktop', sel: true }, { i: 'tablet' }, { i: 'mobile' }].
        map((d, i) =>
        <div key={i} style={{
          ...
          display: 'grid', placeItems: 'center', color: ink2
        }}><Icon name={d.i} size={ICON} /></div>
        )}
      </div>
      <Sep />
      ...
      <div style={{ width: 30, height: 30, display: 'grid', placeItems: 'center', color: ink3, opacity: 0.4 }}>
      ...
      <ViewToggleButton
        ...
        onClick={() => window.__setTweak && window.__setTweak('attachment', tw.attachment === 'docked' ? 'floating' : 'docked')}
        ink2={ink2} dark={pal.dark}
      />
      ...
      <Sep />
      <div style={{ height: 30, padding: '0 12px', display: 'flex', alignItems: 'center',
        borderRadius: 5, background: pal.dark ? '#f1f1f4' : 'var(--ink)', color: pal.dark ? '#1c1c20' : 'var(--paper)',
        fontSize: T.ui, fontWeight: 400 }}>Save</div>
    </div>);
...
function ModeSegment({ pal, tw, ink2, ink3 }) {
  ...
  return (
    <div
      title={title}
      onClick={() => { if (window.__setTweak) window.__setTweak(next); }}
      style={{
      ...
      onMouseEnter={(e) => { e.currentTarget.style.background = pal.dark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)'; }}
  ...
  return (
    <>
      ...
      <div style={{
        ...
        width: 320, background: dark ? '#1c1c20' : 'rgba(255,255,255,0.99)',
        border: dark ? '1px solid rgba(255,255,255,0.12)' : '1px solid rgba(0,0,0,0.12)', borderRadius: 10,
        boxShadow: dark ? '0 16px 40px rgba(0,0,0,0.55)' : '0 16px 40px rgba(0,0,0,0.18)',
        zIndex: 12, padding: 6, fontFamily: 'var(--ui)', fontSize: T.ui,
        color: 'var(--ink)'
      }}>
        <div style={{
          display: 'flex', alignItems: 'center', gap: 8,
        ...
        <div key={i} style={{ height: 1, background: dark ? 'rgba(255,255,255,0.10)' : 'rgba(0,0,0,0.08)', margin: '6px 6px' }} /> :

        <div key={i} style={{
          display: 'flex', alignItems: 'center', gap: 12,
          padding: '8px 10px', borderRadius: 6,
          background: it.sel ? (dark ? 'rgba(255,255,255,0.10)' : 'rgba(0,0,0,0.05)') : 'transparent',
          fontWeight: 400
        }}>
...
function ChromeTabBar({ tab, setTab }) {
  ...
  if (surface === 'translucent') {
    return (
      <div style={{
        ...
        ...(docked ? {} : { borderBottom: dark ? '1px solid rgba(255,255,255,0.10)' : '1px solid rgba(0,0,0,0.08)' })
      }}>
        {!docked && (
          <div style={{ color: dark ? 'rgba(241,241,244,0.45)' : 'var(--ink-3)', opacity: 0.55, marginRight: 2, cursor: 'grab' }}>
            <SixDot />
          </div>
        )}
        <div className="wf-segment" style={{
  ...
  const activeFill = dark ?
    (surface === 'translucent' ? 'rgba(255,255,255,0.16)' : '#26262c') :
    (surface === 'translucent' ? 'rgba(255,255,255,0.45)' : '#fff');
  ...
  const TAB_H = docked ? (toolbarH - 12) : 26, R = 7;
  ...
  return (
    <div style={{
      ...
      paddingLeft: 8, paddingRight: 8, paddingTop: docked ? 12 : 8,
      gap: 0,
      background: stripBg,
      ...
      {!docked && (
        <div style={{
          height: TAB_H, display: 'flex', alignItems: 'center',
          paddingLeft: 2, paddingRight: 8,
          cursor: 'grab', color: dragColor, opacity: 0.55
        }}><SixDot /></div>
      )}
      ...
      {tabs.map((t, idx) => {
        ...
        return (
          ...
          style={{
            ...
            {active &&
            <svg width={TAB_W} height={TAB_H} viewBox={`0 0 ${TAB_W} ${TAB_H}`}
            ...
            <div style={{
              ...
              paddingLeft: R + 10, paddingRight: R + 10,
              fontSize: T.ui, fontWeight: 400,
...
function LeftPanel({ tab, setTab }) {
  ...
  return (
    <div data-panel="left" style={{
      position: 'absolute', left: 18, top: 76, width: 320,
      ...surface,
      borderRadius: 10,
      fontFamily: 'var(--ui)', fontSize: T.ui,
...
function LayersTreePane() {
  ...
  { d: 0, t: 'item', name: 'FeaturedCollection', props: 'modified', i: 'sectionPolaris', g: true, open: true },
  ...
  const visible = treeStyle === 'path' ?
    tree.filter(r => r.t === 'item' && (r.d === 0 || r.sel || r.d === 1 && tree.some(x => x.sel && x.name === 'Hero' && x.d === 0))) :
    tree;

  return (
    <div style={{ padding: 6, fontFamily: 'var(--ui)', fontSize: T.ui }}>
      {visible.map((r, i) => r.t === 'add' ?
      <AddRow key={i} d={r.d} l={r.l} /> :
      <TreeRow key={i} r={r} />
      )}
    </div>);
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      style={{
        ...
        borderRadius: 4,
        fontFamily: 'var(--ui)', fontSize: T.ui, fontWeight: 400,
        cursor: 'pointer'
      }}>
        <div style={{ width: 14 }} />
...
function BlockPicker({ indent }) {
  ...
  return (
    <div style={{
      position: 'absolute', left: indent + 26, top: 30, zIndex: 20,
      width: 260,
      background: dark ? '#1c1c20' : '#fff',
      border: dark ? '1px solid rgba(255,255,255,0.12)' : '1px solid rgba(0,0,0,0.12)', borderRadius: 8,
      boxShadow: dark ? '0 12px 30px rgba(0,0,0,0.50), 0 0 0 1px rgba(255,255,255,0.04)' : '0 12px 30px rgba(0,0,0,0.18), 0 0 0 1px rgba(0,0,0,0.04)',
      fontFamily: 'var(--ui)', fontSize: T.ui, color: 'var(--ink)',
      overflow: 'hidden'
    }}>
      <div style={{
        display: 'flex', alignItems: 'center', gap: 8,
        ...
        <span style={{
          fontSize: T.label, fontFamily: 'var(--mono)', color: 'var(--ink-3)',
          padding: '1px 5px', border: dark ? '1px solid rgba(255,255,255,0.18)' : '1px solid rgba(0,0,0,0.1)', borderRadius: 3
        }}>⌘K</span>
      </div>
      <div style={{ maxHeight: 300, overflowY: 'auto', padding: 4 }}>
        {sections.map((sec) => {
          ...
          return (
            <div key={sec.h}>
              <div style={{
                fontSize: T.label, color: 'var(--ink-3)',
                textTransform: 'uppercase', letterSpacing: 0.6,
                padding: '8px 8px 4px', fontWeight: 500
              }}>{sec.h}</div>
              {items.map((it, k) =>
              <div key={k} style={{
      ...
      <div style={{
        padding: '6px 10px', borderTop: dark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(0,0,0,0.06)',
        display: 'flex', alignItems: 'center', gap: 8,
        fontSize: T.label, color: 'var(--ink-3)'
      }}>
        <span>↑↓ navigate</span>
...
function TreeRow({ r }) {
  ...
  return (
    <div
      ...
      style={{
        display: 'flex', alignItems: 'center', gap: 6,
        paddingLeft: indent, paddingRight: 4,
        height: 28, borderRadius: 4,
        background: r.sel ? (dark ? 'rgba(255,255,255,0.10)' : 'rgba(0,0,0,0.06)') : hover ? (dark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.03)') : 'transparent',
      ...
      {/* name */}
      <div style={{ display: 'flex', alignItems: 'center', flex: 1, overflow: 'hidden', minWidth: 0, whiteSpace: 'nowrap' }}>
        {r.rename ?
        <span style={{
          background: dark ? '#26262c' : '#fff', border: dark ? `1px solid rgba(255,255,255,0.25)` : `1px solid rgba(0,0,0,0.2)`,
          padding: '1px 5px', borderRadius: 3, color: 'var(--ink)', fontWeight: 500
        }}>{r.name}<span style={{ background: 'var(--ink-2)', display: 'inline-block', width: 1, height: 11, marginLeft: 1, verticalAlign: 'middle' }} /></span> :
        treeStyle === 'jsx' ?
        <>
              <span style={{ color: C_TAG, fontFamily: 'var(--mono)', fontSize: T.mono }}>&lt;{r.name}</span>
              {r.props &&
              <span style={{ marginLeft: 4, fontFamily: 'var(--mono)', fontSize: T.mono }}>
                  <span style={{ color: C_ATTR }}> {r.props}</span>
                </span>
              }
              <span style={{ color: C_TAG, fontFamily: 'var(--mono)', fontSize: T.mono, marginLeft: 1 }}>&gt;</span>
            </> :
        ...
        <>
              ...
              {r.props && <span style={{ color: 'var(--ink-3)', marginLeft: 6, fontSize: T.label }}>{r.props}</span>}
            </>
        }
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        <div style={{
          width: 20, height: 20, display: 'grid', placeItems: 'center',
          color: r.hidden ? 'var(--ink-2)' : 'var(--ink-2)', borderRadius: 3, cursor: 'pointer'
        }}>
...
function SettingsPane() {
  return (
    <div style={{ padding: '4px 14px 14px' }}>
      ...
      <Row label="Title"><div className="wf-field" style={{ height: 28, fontSize: T.ui, fontFamily: 'var(--ui)' }}>Home page</div></Row>
      <Row label="Handle"><div className="wf-field" style={{ height: 28, fontSize: T.ui, fontFamily: 'var(--ui)' }}>/</div></Row>
      <Row label="Visible"><div className="wf-toggle" /></Row>
      <SectionHead>SEO</SectionHead>
      <Row label="Meta"><div className="wf-field" style={{ height: 28, fontSize: T.ui, fontFamily: 'var(--ui)' }}>Wines & gifts</div></Row>
      <Row label="Indexable"><div className="wf-toggle" /></Row>
    </div>);
          ...
          {tabs.map(t => {
            ...
            return (
              <div key={t.id}
                ...
                <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontSize: T.ui }}>{t.l}</span>
                <div onClick={(e) => e.stopPropagation()}
                  style={{
                    width: 14, height: 14, borderRadius: 7, marginLeft: 'auto',
        ...
        return (
          ...
          style={{
            ...
            <div style={{
              ...
              paddingLeft: R + 10, paddingRight: R,
              fontFamily: 'var(--ui)', fontSize: T.ui, fontWeight: 400,
              color: activeColor
            }}>
              <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontSize: T.ui }}>{t.l}</span>
              <div
                ...
                style={{
                  ...
                  display: 'grid', placeItems: 'center',
                  color: closeColor, cursor: 'pointer', flexShrink: 0
                }}
                onMouseEnter={(e) => {e.currentTarget.style.background = closeHoverBg;}}
                onMouseLeave={(e) => {e.currentTarget.style.background = 'transparent';}}>
...
function RightPanel() {
  ...
  return (
    <div data-panel="right" style={{
      position: 'absolute', right: 18, top: 76, width: 320,
      ...surface,
      borderRadius: 10,
      fontFamily: 'var(--ui)', fontSize: T.ui,
      color: dark ? '#e6e6ea' : 'var(--ink)',
      ...
      <div style={{ padding: '4px 14px 14px' }}>
        ...
        <Row label="Source"><div className="wf-field" style={{ height: 28, fontSize: T.ui, fontFamily: 'var(--ui)', overflow: 'hidden', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', color: 'var(--ink)' }}>Koop nu:&nbsp;<span style={{ color: '#2c7fd6', background: 'rgba(44,127,214,0.1)', padding: '1px 5px', borderRadius: 3, fontFamily: 'var(--mono)', fontSize: T.label }}>product.name</span></span><span title="Dynamic source" style={{ display: 'grid', placeItems: 'center', color: '#2c7fd6', flexShrink: 0 }}><Icon name="database" size={13} color="#2c7fd6" /></span></div></Row>
        <Row label="Type"><div className="wf-segment" style={{ height: 26, fontSize: T.ui }}><div className="on">Carousel</div><div>Grid</div></div></Row>

        <SectionHead>Layout</SectionHead>
        <Row label="Products"><SliderField value="8" pct={35} dark={dark} /></Row>
        <Row label="Columns"><SliderField value="4" pct={50} dark={dark} /></Row>
        <Row label="Gap"><SliderField value="16" suffix="px" pct={20} dark={dark} /></Row>

        <SectionHead>Spacing</SectionHead>
        <div style={{ position: 'relative', height: 120, background: dark ? '#0e0e12' : '#fff',
          overflow: 'hidden' }}>
          {/* margin frame — outer ring only */}
          <div className="stripes-margin" style={{
            position: 'absolute', inset: 0,
          ...
          <div className="stripes-padding" style={{
            position: 'absolute', inset: 16,
            clipPath: 'polygon(0 0, 100% 0, 100% 100%, 0 100%, 0 0, 16px 16px, 16px calc(100% - 16px), calc(100% - 16px) calc(100% - 16px), calc(100% - 16px) 16px, 16px 16px)'
          }} />
          {/* content */}
          <div style={{ position: 'absolute', inset: 32, background: SEL_FILL_STRONG,
          ...
          <div key={i} className="mono" style={{
            position: 'absolute', ...p, fontSize: T.ui, color: 'var(--ink)', background: dark ? '#26262c' : '#fff',
            padding: '1px 5px', borderRadius: 2, border: dark ? '1px solid rgba(255,255,255,0.12)' : '1px solid rgba(0,0,0,0.08)'
          }}>{p.t}</div>
          )}
        </div>
      </div>
...
function SixDot({ small }) {
  ...
  return (
    <div style={{
      ...
      <div key={i} style={{ width: sz, height: sz, background: 'var(--ink-2)', borderRadius: '50%' }} />
      )}
    </div>);
