  ...
  return (
    <div style={{ position:'relative', width: W, height: H, background:'#e9e4d8' }}>
      ...
      <div className="stripes-margin" style={{
        position:'absolute', left:0, right:0, top: 370, height: 0,
      ...
      <div style={{ position:'absolute', left: 18, top: 92, width: 232, zIndex: 5 }}>
        <FloatingPanel title="Layers" sub="home / template" width={232} height={460}>
          <div style={{ padding: '8px 6px', overflow:'hidden', height:'100%' }} className="no-scrollbar">
            {[
              ...
              <div key={i} style={{
                ...
                <div style={{
                  width: 12, height: 12, border: '1px solid var(--ink-3)', borderRadius: 2,
      ...
      <div style={{ position:'absolute', right: 18, top: 92, width: 280, zIndex: 5 }}>
        <FloatingPanel title="Hero" sub="section.hero" width={280} height={500}
          footer={<><Glyph kind="grip" size={10} /><span>drag · cmd+. to dismiss</span></>}>
          <div style={{ padding: 12, overflow:'hidden', height:'100%' }} className="no-scrollbar">
            <SectionHead>Content</SectionHead>
            <Row label="Heading"><div className="wf-field">Passion for…</div></Row>
            <Row label="Subhead"><div className="wf-field">tagline copy</div></Row>
            <Row label="CTA"><div className="wf-field">Shop now</div></Row>

            <SectionHead>Layout</SectionHead>
            <Row label="Width">
              <div className="wf-segment"><div>Page</div><div className="on">Full</div></div>
            </Row>
            <Row label="Align">
              <div className="wf-segment"><div>Left</div><div className="on">Center</div><div>Right</div></div>
            </Row>
            <Row label="Padding Y" hint="80 px"><div className="wf-slider" /></Row>
            <Row label="Min height" hint="300 px"><div className="wf-slider" /></Row>

            <SectionHead>Background</SectionHead>
            <Row label="Type">
              <div className="wf-segment"><div className="on">Color</div><div>Image</div><div>Video</div></div>
            </Row>
            <Row label="Color"><div style={{ display:'flex', gap: 6, alignItems:'center' }}>
              <div style={{ width: 18, height: 18, borderRadius: 4, background:'#d9d4c5', border:'1px solid var(--line)' }} />
              <div className="mono" style={{ fontSize: 11 }}>#d9d4c5</div>
            </div></Row>
          </div>
        </FloatingPanel>
      </div>
      ...
      <div style={{ position:'absolute', left: '50%', top: 460, transform:'translateX(-50%)', zIndex: 6 }}>
        <div style={{
          width: 26, height: 26, borderRadius:'50%',
          background: 'var(--insp-blue-strong)', color:'#fff',
          display:'grid', placeItems:'center',
          boxShadow:'0 4px 12px rgba(44,127,214,0.45)',
        }}><Glyph kind="plus" size={12} color="#fff" /></div>
      </div>
      ...
      <div style={{ position:'absolute', left: 36, top: 70 + 300 - 28, zIndex: 6 }}>
        <div style={{
          ...
          height: 26, paddingRight: 4,
        }}>
          <div style={{ padding:'0 8px', borderRight:'1px solid rgba(255,255,255,0.15)', height:'100%', display:'flex', alignItems:'center' }}>
            <span style={{ color:'#f5b9b9' }}>section</span>
            <span style={{ color:'#6cb1f0' }}>.hero</span>
          </div>
          {['↑','↓','⎘','⌫'].map((c,i) => (
            <div key={i} style={{ width: 22, height: 22, display:'grid', placeItems:'center' }}>{c}</div>
          ))}
        </div>
      </div>

      {/* annotation */}
      <Annotation x={W/2 - 100} y={H - 56}
...
function Annotation({ x, y, text, w = 240, dir = 'down' }) {
  return (
    <div style={{ position:'absolute', left: x, top: y, width: w, zIndex: 10 }}>
      <div style={{ fontFamily:'var(--hand)', fontSize: 17, color:'var(--ink-2)', lineHeight: 1.15 }}>{text}</div>
    </div>
  );
}
