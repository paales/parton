...
window.TweaksCtx = React.createContext({ palette: 'inspector', treeStyle: 'plain', surface: 'light', toolbar: 'floating', attachment: 'floating' });
  ...
  return (
    ...
    <DesignCanvas>
      <DCSection id="consolidated"
        ...
        <DCArtboard id="v6" label="F · Consolidated — JSX tree, tabs, page navigator, breadcrumb, exit" width={1440} height={900}>
          <V6Consolidated />
        </DCArtboard>
      </DCSection>

      <DCSection id="floating-block-editor"
        ...
        <DCArtboard id="v1" label="A · Floating Island — both panels float, fully glassy" width={1100} height={720}>
          <V1FloatingIsland />
        </DCArtboard>
        <DCArtboard id="v2" label="B · Edge Rails — collapsed left rail, fly-out layers, persistent right drawer" width={1100} height={720}>
          <V2EdgeRails />
        </DCArtboard>
        <DCArtboard id="v3" label="C · Radial / Contextual — no chrome, action wheel orbits selection" width={1100} height={720}>
          <V3Radial />
        </DCArtboard>
        <DCArtboard id="v4" label="D · Command-bar first — ⌘K is primary, spec card hovers near selection" width={1100} height={720}>
          <V4CommandBar />
        </DCArtboard>
        <DCArtboard id="v5" label="E · Inspector Overlay — devtools-DNA at full strength: tabs, DOM tree, box model" width={1100} height={720}>
          <V5OverlayCanvas />
        </DCArtboard>
      </DCSection>
      ...
      <DCSection id="legend"
        ...
        <DCArtboard id="vocab" label="Vocabulary — same in every variation" width={900} height={360}>
          <Vocabulary />
        </DCArtboard>
      </DCSection>
    </DesignCanvas>
    ...
    <TweaksPanel title="Tweaks">
      ...
      <div style={{ fontSize: 10.5, color: 'rgba(41,38,27,.5)', marginTop: -4 }}>
        {t.attachment === 'floating' && 'Drift above the canvas with shadows and rounded corners.'}
        {t.attachment === 'docked'   && 'Snap flush to the edges, full height; topbar pins top-right.'}
      </div>
    </TweaksPanel>
    ...
    <div style={{ padding: 30, background:'#f4f1ea', height:'100%', fontFamily:'var(--ui)' }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap: 18 }}>
        ...
        <Swatch title="Content / selection" sub="dashed blue, light blue fill">
          <div style={{ width:'100%', height: 80, outline:'1.5px dashed var(--insp-blue-strong)',
                       outlineOffset:-1, background:'rgba(108,177,240,0.18)', borderRadius: 0 }} />
        </Swatch>
      ...
      <div style={{ marginTop: 24, fontFamily:'var(--hand)', fontSize: 18, color:'var(--ink-2)', lineHeight: 1.3 }}>
        every variation reuses these four primitives — the editor never feels like a separate UI sitting on top,<br/>
...
function Swatch({ title, sub, children }) {
  return (
    <div>
      <div style={{ background:'#fff', border:'1px solid rgba(0,0,0,0.12)', borderRadius: 8, padding: 12 }}>
        {children}
      </div>
