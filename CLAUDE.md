...
- **Four constructors, one engine.** Pick by role:
  ...
  - `scroller({name, range, item, leaf, className})` — a windowed collection: one CSS grid, a placed span of cull-gated leaf partons around the `?page=` anchor, reservation shells for the rest. Geometry is three CSS variables (`--scroller-cols/-row/-gap`); `item` renders one grid cell (its own parton for per-entity invalidation); pages are a projection, never a render unit.
