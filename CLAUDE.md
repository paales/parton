...
| **Scroller**            | Windowed collections: one CSS grid, a placed span of cull-gated leaf partons around the anchor, reservation shells for the rest (pure CSS arithmetic — geometry is three app variables); a scrollbar jump paints locally and moves the span with one statement. | `reference/scroller.md`                                       |
...
- **Four constructors, one engine.** Pick by role:
  ...
  - `scroller({name, load, key, render, leaf, ring})` — a windowed collection: one CSS grid, a placed span of cull-gated leaf partons around the `?page=` anchor, reservation shells for the rest. Geometry is three CSS variables (`--scroller-cols/-row/-gap`). `load` defines the item, `key` names it (the entity key), `render` draws it — a COMPONENT over `{item, index, id?}`, so a parton component passes directly (its own parton for per-entity invalidation); pages are a projection, never a render unit.
...
| Folder                                 | For                                                                                                                                                                                                                                                                                                        |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`docs/reference/`](./docs/reference/) | Framework contracts. `intro` · `partial` · `block` · `scroller` · `cells` · `frames-navigation` · `remote-frame` · `cache` · `cms` · `prior-art`. Read these to USE the framework.                                                                                                                                      |
