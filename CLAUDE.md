...
| **Scroller**            | Windowed collections: an interval tree of cullable partons over item index space — leaves resolve slices in view, culled regions collapse to shells, the anchor param seeds cold renders and mirrors scroll.                   | `reference/scroller.md`                                       |
...
- **Four constructors, one engine.** Pick by role:
  ...
  - `scroller(Render, {range, shell, estimate, …})` — a windowed collection: an interval tree of cullable partons over item index space; Render is the slice's layout renderer, items are their own partons, `anchor` is the cold seed + URL mirror (pages are a projection, never a render unit).
