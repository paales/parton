	...
	const types: SchemaObjectType[] = [
		{
			...
			fields: [
				...
				{ name: "is_legendary", type: { kind: "SCALAR", name: "Boolean", ofType: null }, args: [] },
				{ name: "base_experience", type: { kind: "SCALAR", name: "Int", ofType: null }, args: [] },
				{ name: "sprites", type: { kind: "OBJECT", name: "Sprites", ofType: null }, args: [] },
				{ name: "types", type: { kind: "LIST", name: null, ofType: { kind: "OBJECT", name: "PokemonType", ofType: null } }, args: [] },
				{ name: "abilities", type: { kind: "LIST", name: null, ofType: { kind: "OBJECT", name: "Ability", ofType: null } }, args: [] },
				{ name: "species", type: { kind: "OBJECT", name: "Species", ofType: null }, args: [] },
			],
		},
		{
			name: "Sprites",
		...
		{ name: "Type", fields: [{ name: "name", type: { kind: "SCALAR", name: "String", ofType: null }, args: [] }] },
		{
			...
			fields: [
				{ name: "name", type: { kind: "SCALAR", name: "String", ofType: null }, args: [] },
				{ name: "is_hidden", type: { kind: "SCALAR", name: "Boolean", ofType: null }, args: [] },
			],
		},
		{
			name: "Species",
			fields: [
				{ name: "name", type: { kind: "SCALAR", name: "String", ofType: null }, args: [] },
				{ name: "generation", type: { kind: "OBJECT", name: "Generation", ofType: null }, args: [] },
			],
		},
		{ name: "Generation", fields: [{ name: "name", type: { kind: "SCALAR", name: "String", ofType: null }, args: [] }] },
	];
...
describe("Edge cases — conditional access", () => {
	...
	it("only records accessed branches", () => {
		...
		const pokemon = createProxy(schema, "Pokemon", recorder, {
			is_legendary: false, base_experience: 64, name: "bulbasaur",
		}) as any;
		pokemon.name.value;
...
describe("Edge cases — array operations", () => {
	...
	it(".find()", () => {
		...
		expect(pokemon.abilities.find((a: any) => a.is_hidden.value === true).name.value).toBe("chlorophyll");
	});
	...
	it("nested arrays in discovery", () => {
		...
		p.types.map((t: any) => { t.slot.value; t.type.name.value; });
		p.abilities.map((a: any) => { a.name.value; a.is_hidden.value; });
...
describe("Edge cases — raw() expressions", () => {
	it("passes raw without quotes", () => {
		const query = compileQuery([{ field: "name", children: [] }], "pokemon", {
			where: raw("{id: {_eq: 25}}"), order_by: raw("{name: asc}"),
		});
	...
	it("handles nested object args", () => {
		const query = compileQuery([{ field: "name", children: [] }], "pokemon", { where: { id: { _eq: 25 } } });
		expect(query).toContain("where: {id: {_eq: 25}}");
	});
});
...
describe("Edge cases — thenable/use()", () => {
	...
	it("use(pokemon) resolves to full data", async () => {
		const pokemon = createProxy(schema, "Pokemon", new AccessRecorder(), { name: "pikachu", id: 25 }) as any;
		expect(await pokemon).toEqual({ name: "pikachu", id: 25 });
	});

	it("use(pokemon.name) resolves to scalar", async () => {
		const pokemon = createProxy(schema, "Pokemon", new AccessRecorder(), { name: "pikachu" }) as any;
	...
	it("use(pokemon.species) resolves to nested object", async () => {
		...
		expect(await pokemon.species).toEqual({ name: "pikachu", generation: { name: "gen-1" } });
	});
});
