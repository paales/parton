...
function createTestSchema(): SchemaGraph {
	const types: SchemaObjectType[] = [
		...
		{ name: "Sprites", fields: [{ name: "front_default", type: { kind: "SCALAR", name: "String", ofType: null }, args: [] }] },
		{
			name: "PokemonType",
			fields: [
...
describe("renderForDiscovery", () => {
	...
	it("discovers accesses from JSX components", () => {
		function Card({ pokemon }: { pokemon: any }) {
			return <div>{pokemon.name.value} #{pokemon.id.value}</div>;
		}
	...
	it("discovers through nested component tree", () => {
		function TypeBadge({ type }: { type: any }) { return <span>{type.type.name.value}</span>; }
		function Card({ pokemon }: { pokemon: any }) {
			return (
				<div>
					<h1>{pokemon.name.value}</h1>
					{pokemon.types.map((t: any) => <TypeBadge key={t.slot.value} type={t} />)}
				</div>
			);
		}
		const recorder = new AccessRecorder();
	...
	it("discovers across sibling partials", () => {
		...
		function Page({ pokemon }: { pokemon: any }) {
			return <div><Hero pokemon={pokemon} /><Sprite pokemon={pokemon} /><Species pokemon={pokemon} /></div>;
		}
		const recorder = new AccessRecorder();
	...
	it("returns void", () => {
		function Card({ pokemon }: { pokemon: any }) { return <div>{pokemon.name.value}</div>; }
		const recorder = new AccessRecorder();
		expect(renderForDiscovery(<Card pokemon={createProxy(schema, "Pokemon", recorder)} />)).toBeUndefined();
	});

	it("full controller pattern", () => {
		function Card({ pokemon }: { pokemon: any }) {
			return <div>#{pokemon.id.value} {pokemon.name.value} <img src={pokemon.sprites.front_default.value} /></div>;
		}
		function Page({ pokemonList }: { pokemonList: any }) {
			return <div>{pokemonList.map((p: any) => <Card key={p.id.value} pokemon={p} />)}</div>;
		}
		...
		const dataProxy = createProxy(schema, "Pokemon", new AccessRecorder(), {
			id: 25, name: "pikachu", sprites: { front_default: "https://example.com/pikachu.png" },
		});
	...
	it("handles components that throw", () => {
		function Broken({ pokemon }: { pokemon: any }) { pokemon.name.value; throw new Error("boom"); }
		const recorder = new AccessRecorder();
	...
	it("discovers through throwing wrapper (client component pattern)", () => {
		...
		function Card({ pokemon }: { pokemon: any }) {
			return <div>{pokemon.name.value} - {pokemon.height.value}</div>;
		}
