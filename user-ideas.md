...
- How is data in a CMS dimensional that is not part of the default request? Data changes over time, time traveling cms.
- Is Data it's self a dimension? A blog list _IS_ data but also is CMS configuration? Entity data is required, blog title, intro, date, author, but actual blog page isn't. META and whatever gets designed, with a preset that is just a field?
- How is this block editor a wysiwyg?

- Storage following the getPathname than the query params

- I've seen an important bit; a limitation or perhaps something we should be considering properly: For the selector-targeted refetch to actually pick up the new ?select= value, getSearchParam("select") needs to be read inside the tree/field Partials (not at the page level). Right now the editor reads selected on the outer page and threads it as a prop — that bakes the value into the snapshot at render time, so cache-mode refetches stay stale. Let me push the accessor reads into the partials themselves.

- How does the editor mode and cache interact with each other? I hate that platforms have a 'disable cache' or 'bypass cache' functionality and putting the system into shit mode, cache should a transparent internal primitive and we shouldn't have to manually flush anything. For example in the CMS mode, does that mean that cached partials need to be flushed? If they depend on cms data they will automatically get a new version, so that is tackled. Data caching is not a CMS concern. Data handler is a chache concern? Flush by tag, in the editor sidebar we might get a collapsed cache section that allows us to flush #ids or .tags?

- The tests do not properly handle multiple concurrent RSC streams it seems and when running parallel they seem to fail. Is there context / cache pollution that is causing this? This is probably something the framework should handle properly? I see many '[vite] program reloaded' console messages.
