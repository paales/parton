...
/**
 * Bare streaming test — cache mode.
 *
 * Same three-stage Suspense tree as bare-stream, but refetches via
 * ?partials=bare/stage-1,bare/stage-2,bare/stage-3 to trigger cache-mode
 * refetch (server wraps each fresh partial in Suspense, client
 * PartialsClient fills template from cache).
 *
 * If this reveals progressively → cache mode works, pokemon page is doing
 * something else wrong.
 * If batched at ~2s → cache mode breaks streaming, confirming the culprit.
 */
test("bare streaming (cache mode): stages reveal progressively", async ({ page }) => {
  const consoleLines: string[] = [];
  page.on("console", (msg) => {
    const t = msg.text();
    if (t.includes("[stream]") || t.includes("[cache-mode]")) consoleLines.push(t);
  });
  ...
  await page.locator('[data-testid="refetch-cache"]').click();
