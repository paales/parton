...
test.describe("notFound + redirect", () => {
  test("notFound() synchronously: HTML returns 404 + NotFoundPage body", async ({
    page,
  }) => {
  ...
  test("notFound() from deep async server component: HTML still returns 404", async ({
    page,
  }) => {
    ...
    expect(
      response?.status(),
      "async notFound should surface as HTTP 404",
    ).toBe(404);
    await expect(page.locator('[data-testid="not-found"]')).toBeVisible({
      timeout: 10000,
    });
  ...
  test("redirect(): HTML request follows 302 to destination", async ({
    page,
  }) => {
