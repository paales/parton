...
test("refresh-all-prices updates ticks after /magento cache is warm", async ({
  page,
}) => {
  ...
  await page.waitForSelector('[data-testid^="live-price-"][data-price-tick]', { timeout: 15000 });
...
test("dynamic Partials inside a cached region render fresh across full loads", async ({
  page,
}) => {
