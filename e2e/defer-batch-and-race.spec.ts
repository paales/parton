...
test.describe("defer batching + race", () => {
  test("two <WhenStored> pre-set keys coalesce into one RSC request", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="batch-a-fallback"]'),
    ).toHaveCount(0, { timeout: 10000 });
    await expect(
      page.locator('[data-testid="batch-b-fallback"]'),
    ).toHaveCount(0, { timeout: 10000 });
  ...
  test("deferred activation doesn't wait for a slow suspending sibling", async ({
    page,
  }) => {
    ...
    console.log(
      `fallback=${fallbackMs}ms race-defer=${raceMs}ms slow-content=${slowMs}ms`,
    );
