...
test.describe("concurrent refetches", () => {
  test("single refetch updates its partial (baseline)", async ({ page }) => {
    ...
    const before = await page
      .locator('[data-testid="concurrent-a"]')
      .textContent();
    ...
    await expect
      .poll(
        async () =>
          await page.locator('[data-testid="concurrent-a"]').textContent(),
        { timeout: 5000 },
      )
      .not.toBe(before);
  });

  test("three distinct-id refetches run in parallel server-side", async ({
    page,
  }) => {
    ...
    console.log(
      `RSC calls after concurrent clicks (${totalMs}ms total):`,
      JSON.stringify(rscCalls),
    );
    ...
    const idsPerCall = rscCalls.map(
      (c) => c.partials?.split(",").filter(Boolean) ?? [],
    );
  ...
  test("rapid-fire refetches against the SAME id: last completion wins", async ({
    page,
  }) => {
    await page.goto("/defer-demo");
    await page.waitForSelector('[data-testid="concurrent-c"]', {
    ...
    const final = await page
      .locator('[data-testid="concurrent-c"]')
      .textContent();
