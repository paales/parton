...
test("editor mode + magento + refresh prices does not throw HoistingViolation", async ({
  ...
  await context.addCookies([
    { name: "__editor", value: "1", url: "http://localhost:5179" },
  ]);
  ...
  const errorCardVisible = await page
    .locator("text=/failed to render/i")
    .count();
  expect(errorCardVisible, "no PartialErrorBoundary card should be visible").toBe(0);

  const violations = errors.filter((e) =>
    e.includes("HoistingViolation") || e.includes("manifest captured"),
  );
