...
test("RefreshPriceButton instance survives a price refetch", async ({
  page,
}) => {
  page.on("pageerror", (err) => console.log("PAGE ERROR:", err.message));

  await page.goto("/magento");
  await page.waitForSelector('[data-testid^="live-price-"]', { timeout: 15000 });

  // Tag every refresh-price button so we can detect remount.
  await page.evaluate(() => {
    document
      .querySelectorAll<HTMLElement>('[data-testid^="refresh-price-"]')
      .forEach((btn, i) => {
        (btn as any).__instanceId = `inst-${i}-${Math.random()
          .toString(36)
          .slice(2, 8)}`;
      });
  });
  ...
  const before = await page.evaluate(() => {
    ...
    document
      .querySelectorAll<HTMLElement>('[data-testid^="refresh-price-"]')
      .forEach((btn) => {
        out[btn.getAttribute("data-testid")!] =
          (btn as any).__instanceId ?? "NONE";
      });
    return out;
  });
  ...
  for (const [k, v] of Object.entries(before)) {
    ...
    expect(
      after[k],
      `Sibling button ${k} remounted during unrelated refetch`,
    ).toBe(v);
  }
});
