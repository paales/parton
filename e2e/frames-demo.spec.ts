...
test("main listing updates the window URL (useNavigation falls through to window)", async ({
  ...
  await expect(page.getByTestId("main-detail")).toHaveAttribute(
    "data-sku",
    "alpha",
  );
...
test("browser back on a main-listing navigation returns to the list", async ({
  page,
}) => {
  await page.goto("/frames-demo");
  await awaitHydrated(page);
...
test("reload re-renders the frame without changing its URL", async ({
  page,
}) => {
  await page.goto("/frames-demo");
  await awaitHydrated(page);
