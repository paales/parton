 ...
 * so you can watch chunks pop in as you scroll). The pulse cell
 * resolves HERE — a pulse bump re-sends the whole chunk as one lane,
 * and the light flashes when ITS bytes arrive; a culled chunk
 * resolves nothing and is dark to the cell's invalidation until it
 * flips back in (the cell keeps counting; re-entry renders the
...
export function defineWorldChunk(geo: WorldGeometry): ChunkComponent {
  ...
  return parton(
    async function WorldChunkRender({ cx, cy }: ChunkPos & RenderArgs) {
      pulse.ensureTicker(cx, cy)
      const value = await pulse.cell.resolve({ cx, cy })
      return (
        <div className="chunk" data-testid={`chunk-${cx},${cy}`} data-loaded>
          <span className="chunk__coord">
            {cx},{cy}
          </span>
          <ActivityLight ck={`${cx},${cy}`} stamp={value.value} />
          <span className="chunk__pulse">{value.value}ms</span>
