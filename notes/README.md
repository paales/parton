...
| `AUTO_TRACKED_CACHE_KEYS.md`  | Auto-derived cache keys via tracked request accessors (`getCookie` / `getHeader` / `getSearchParam` / `getPathname`) with a Cache-Control-shaped `cache` prop (`{maxAge, staleWhileRevalidate, vary?, bypass?}`). Accessors hoist like React hooks; conditional reads throw `HoistingViolationError`. **Status: implemented.** Predecessor design notes are in `/archive/` (`SERVER_CACHE_NOTES.md`, `PARTIAL_CACHE_DESIGN.md`). |
