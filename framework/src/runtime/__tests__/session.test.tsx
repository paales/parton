 ...
 * empty scene.
 *
 * The behavioral suite runs over EVERY `SessionStore` implementation
 * (memory and SQLite): the policy layer in session.ts — idle-TTL
 * expiry, touch-on-activity, the rate-limited sweep — is
 * backend-independent by construction, and the parameterization is
 * what proves the seam holds. SQLite-only concerns (restart
 * persistence, the shared database handle) get their own describe at
 * the end.
 */
...
const sqliteDirs = new WeakMap<SqliteSessionStore, string>()

const stores: StoreCase[] = [
  {
    name: "MemorySessionStore",
    make: () => new MemorySessionStore(),
    cleanup: () => {},
  },
  {
    name: "SqliteSessionStore",
    make: () => {
      const dir = mkdtempSync(join(tmpdir(), "parton-sessions-"))
      const store = new SqliteSessionStore(join(dir, "sessions.db"))
      sqliteDirs.set(store, dir)
      return store
    },
    cleanup: (store) => {
      const s = store as SqliteSessionStore
      s.close()
      const dir = sqliteDirs.get(s)
...
describe.each(stores)("over $name", ({ make, cleanup }) => {
...
describe("SqliteSessionStore — restart persistence", () => {
  let dir: string

  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "parton-sessions-restart-"))
  })
  ...
  it("a session written before a restart is served after it (new store, same file)", async () => {
    const path = join(dir, "sessions.db")
    const a = new SqliteSessionStore(path)
    setSessionStore(a)
    const { cookies } = await runWithRequestAsync(await makeRequest(), async () => {
      setSessionFrameUrl(["cart"], "/cart/checkout")
    })
    ...
    const b = new SqliteSessionStore(path)
    setSessionStore(b)
    const { result } = await runWithRequestAsync(await makeRequest(sid), async () =>
      getSessionFrameUrl(["cart"]),
    )
  ...
  it("shares one database file with the cell adapter via its db handle", async () => {
    const cells = new SqliteCellStorage(join(dir, "parton.db"))
    const sessions = new SqliteSessionStore(cells.db)
    setSessionStore(sessions)
    cells.write("default", "c", "p1", 1)
    await runWithRequestAsync(await makeRequest(), async () => {
      setSessionFrameUrl(["cart"], "/x")
    })
    // Both tables live in the one file, and both surfaces read back.
    expect(cells.read("default", "c", "p1")).toBe(1)
    expect(_sessionStats().sessions).toBe(1)
    cells.close() // closes the shared handle
  })
})
