 ...
 *
 * The whole suite runs over EVERY persistent backend (memory and
 * SQLite) — the ts contract is what `setCellStorage` promises, not an
 * implementation detail of one adapter, and the restart-equivalence
 * test is the adapter conformance gate.
 */
...
interface BackendCase {
  name: string
  make: () => CellStorage
  cleanup: (backend: CellStorage) => void
}

const sqliteDirs = new WeakMap<SqliteCellStorage, string>()

const backends: BackendCase[] = [
  {
    name: "MemoryCellStorage",
    make: () => new MemoryCellStorage(),
    cleanup: () => {},
  },
  {
    name: "SqliteCellStorage",
    make: () => {
      const dir = mkdtempSync(join(tmpdir(), "parton-cells-tsp-"))
      const storage = new SqliteCellStorage(join(dir, "cells.db"))
      sqliteDirs.set(storage, dir)
      return storage
    },
    cleanup: (backend) => {
      const storage = backend as SqliteCellStorage
      storage.close()
      const dir = sqliteDirs.get(storage)
      if (dir) rmSync(dir, { recursive: true, force: true })
    },
  },
]

describe.each(backends)("over $name", ({ make, cleanup }) => {
  let backend: CellStorage

  beforeEach(() => {
    backend = make()
    setCellStorage(backend)
    _clearInvalidationRegistry()
  })
...
describe("SqliteCellStorage — database round-trip", () => {
  let dir: string
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "parton-cells-sq-"))
  })
  afterEach(() => {
    rmSync(dir, { recursive: true, force: true })
  })

  it("an invalidated (undefined-value) row keeps its ts across reopen", () => {
    const path = join(dir, "cells.db")
    const a = new SqliteCellStorage(path)
    a.write("default", "inv.cell", "pk1", undefined) // invalidate() shape
    a.stampTs("default", "inv.cell", "pk1", 9)
    a.close()
    const b = new SqliteCellStorage(path)
    expect(b.read("default", "inv.cell", "pk1")).toBeUndefined()
    expect(b.readTs("default", "inv.cell", "pk1")).toBe(9)
    expect(b.maxTs()).toBe(9)
    // The surviving row still accepts a forward stamp.
    b.stampTs("default", "inv.cell", "pk1", 12)
    expect(b.readTs("default", "inv.cell", "pk1")).toBe(12)
    b.close()
  })
