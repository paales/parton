 ...
 */
export function partonChannelServer(
	options: PartonChannelServerOptions = {},
): Plugin {
...
async function loadDevHandler(
	...
	const source = entrySource(
		(environment.config as { build?: { rollupOptions?: { input?: unknown } } })
			.build?.rollupOptions?.input,
		entryName,
	);
	if (!source) {
		throw new Error(
			`[parton:channel-server] no rsc build input named '${entryName}'`,
		);
	}
	const resolved = await environment.pluginContainer.resolveId(source);
	if (!resolved) {
		throw new Error(
			`[parton:channel-server] cannot resolve rsc entry '${source}'`,
		);
	}
