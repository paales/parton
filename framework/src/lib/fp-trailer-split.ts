...
class SegmentIterator implements AsyncIterator<Segment> {
  ...
  constructor(source: ReadableStream<Uint8Array>, signal?: AbortSignal, onEntry?: OnWireEntry) {
  ...
  private async classifyNext(): Promise<"payload" | "lanes" | "end"> {
    while (true) {
      if (this.leftover.length === 0) {
        ...
        if (chunk == null) return this.leftover.length === 0 ? "end" : "payload"
    ...
    try {
      while (true) {
        ...
        if (parsed === "need-more") {
          ...
          if (this.leftover.length > 0) torn("lanes segment ended mid-frame header")
          else if (open.size > 0) torn("connection closed with parton lanes open", true)
          this.exhausted = true
          return
        }
        if (parsed === "invalid") {
        ...
        if (parsed.tag === "next") {
          ...
          if (open.size > 0) torn("lanes segment ended with parton lanes open", true)
          return
        }
