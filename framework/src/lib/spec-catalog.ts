 ...
 *    descendant's `vary` against the current request to keep
 ...
 * vary callback, and match pattern. CMS-specific block metadata
 ...
 *  `VaryScope` in partial.tsx. */
export type SpecCatalogVary = (scope: {
  url: URL
  pathname: string
...
export interface SpecCatalogEntry {
   ...
   *  against the current request without rendering. */
  vary?: SpecCatalogVary
