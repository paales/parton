 ...
 * as one reserved entry, `ParentContext`. Read it with
 * `getServerContext(ParentContext)`; scope descendants by rendering them
 * inside `<ParentContext value={childCtx}>` — the parton parent is the
 * framework's own first consumer of `createServerContext`.
 */

import { createServerContext } from "./server-context.ts"
...
// knowledge: server-context owns the mechanism, this is a consumer. A parton
// (and `<Frame>`, and the cache's isolated render) scopes its descendants by
// rendering them inside `<ParentContext value={childCtx}>`; because the
// provider overlays only this key, user server contexts thread through to the
// descendants untouched.

export const ParentContext = createServerContext<PartialCtx>(ROOT)
