...
async function consumeAll(source: ReadableStream<Uint8Array>, signal?: AbortSignal) {
...
describe("splitSegments — lanes segments", () => {
  ...
  it("a source close with a lane open errors that lane's body", async () => {
    ...
    if (first.done || first.value.kind !== "payload") throw new Error("expected payload")
    await collect(first.value.body)
    await first.value.trailers

    const second = await iter.next()
    if (second.done || second.value.kind !== "lanes") throw new Error("expected lanes")
  ...
  it("an abort during a lanes segment cancels immediately", async () => {
    ...
    const hanging = new ReadableStream<Uint8Array>({
      start(controller) {
        controller.enqueue(
          bytes("seg-0", SETTLED, NEXT, LANES, muxFrame("solo", encoder.encode("x"))),
        )
        // never closes
      },
      cancel() {
  ...
  it("payload segments still parse untouched around a lanes region", async () => {
    ...
    const segments = await consumeAll(streamOf(wire))
    expect(segments.map((s) => s.kind)).toEqual(["payload", "lanes", "payload"])
