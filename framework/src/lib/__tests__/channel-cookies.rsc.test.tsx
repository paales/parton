...
describe("cookie frame endpoint apply", () => {
  it("applies a cookie frame to the session overlay and queues the re-lane; a delete tombstones", async () => {
    ...
    try {
      ...
      expect(takeConnectionCookieChanges(session)).toEqual(new Set(["theme", "cart_id"]))
  ...
  it("a cookie frame re-lanes exactly the reader with the fresh value; a non-reader never re-renders", async () => {
    ...
    await withLiveDrive(
      ...
      async (h) => {
        ...
        expect((await decodeLane(laneDark)).bodyText).toContain("theme:dark:2")
        ...
        expect((await decodeLane(laneLight)).bodyText).toContain("theme:light:3")
        ...
        expect((await decodeLane(laneGone)).bodyText).toContain("theme:none:4")
        expect(renders.plain).toBe(1)
