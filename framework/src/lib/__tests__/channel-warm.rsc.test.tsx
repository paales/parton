	...
	it("is latest-wins and never renders — no body run, no delivery seq from telemetry alone", async () => {
		...
		await withLiveDrive(
			...
			async (h) => {
				...
				const seqEntries = (): number =>
					h.entries.filter((e) => e.tag === "seq").length;
		...
		await withLiveDrive(
			...
			async (h) => {
				...
				expect(
					await post(
						scope,
						flipInEnvelope(conn, 2, "cull-warm", ["cull-warm"]),
					),
				).toBe(204);
				...
				expect(
					await post(
						scope,
						flipInEnvelope(conn, 3, "cull-cold", ["cull-warm", "cull-cold"]),
					),
				).toBe(204);
