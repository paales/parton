...
describe("channel envelope decode", () => {
	...
	it("rejects malformed envelopes and malformed KNOWN-kind frames", () => {
		...
		expect(
			decodeChannelEnvelope({ connection: "", seq: 1, frames: [] }),
		).toBeNull();
		expect(
			decodeChannelEnvelope({ connection: "c", seq: Number.NaN, frames: [] }),
		).toBeNull();
		expect(
			decodeChannelEnvelope({ connection: "c", seq: 1, frames: [{}] }),
		).toBeNull();
...
describe("channel endpoint", () => {
	it("maps apply / unknown-connection / malformed to 204 / 404 / 400", async () => {
		...
		try {
			const ok = await post(
				envelope("chan-http", 1, [
					{ kind: "visible", changed: ["a"], visible: ["a"], cached: [] },
				]),
			);
			...
			expect(
				(
					await post(
						envelope("chan-http", 2, [
							{ kind: "visible", changed: "nope", visible: [] },
						]),
					)
				).status,
			).toBe(400);
	...
	it("rejects cross-site provenance with 403; same-origin and header-less pass", async () => {
		...
		try {
			const body = envelope("chan-origin", 1, [
				{ kind: "visible", changed: ["a"], visible: ["a"] },
			]);
			expect(
				(await post(body, { origin: "http://evil.example" })).status,
			).toBe(403);
			expect(
				(await post(body, { "sec-fetch-site": "cross-site" })).status,
			).toBe(403);
			expect(
				(await post(body, { "sec-fetch-site": "same-site" })).status,
			).toBe(403);
	...
	it("an envelope resolving a different scope than the attach answers 404", async () => {
		...
		try {
			const body = envelope("chan-scope", 1, [
				{ kind: "visible", changed: ["a"], visible: ["a"] },
			]);
			expect(
				(await post(body, { "x-test-scope": "worker-other" })).status,
			).toBe(404);
			expect(session.pendingFlips.size).toBe(0);
			expect(
				(await post(body, { "x-test-scope": "worker-attach" })).status,
			).toBe(204);
			expect(session.pendingFlips.size).toBe(1);
		} finally {
	...
	it("an envelope presenting a different session identity than the attach answers 404", async () => {
		...
		try {
			...
			expect(
				(await post(boundBody, { cookie: "__frame_sid=sid-other" })).status,
			).toBe(404);
			expect(bound.pendingFlips.size).toBe(0);
			// The attach's own identity applies.
			expect(
				(await post(boundBody, { cookie: "__frame_sid=sid-attach" })).status,
			).toBe(204);
			...
			expect(
				(await post(anonBody, { cookie: "__frame_sid=sid-late" })).status,
			).toBe(404);
		} finally {
