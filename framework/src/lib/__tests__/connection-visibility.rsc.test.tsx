...
describe("connection-session visibility", () => {
  it("the attach seed drives the first segment; in-flips lane, out-flips don't", async () => {
    ...
    await withLiveDrive(
      ...
      async (h) => {
        ...
        expect(await postVisible(scope, conn, 1, ["cull-b"], ["cull-a", "cull-b"])).toBe(204)
        ...
        expect(await postVisible(scope, conn, 2, ["cull-b"], ["cull-a"])).toBe(204)
        ...
        expect(await postVisible(scope, conn, 3, ["cull-b"], ["cull-a", "cull-b"])).toBe(204)
  ...
  it("a flip for an id the route never rendered defers without disturbing the stream", async () => {
    ...
    await withLiveDrive(
      ...
      async (h) => {
        ...
        // the same connection.
        expect(
          await postVisible(scope, conn, 1, ["not-on-this-route"], ["not-on-this-route"]),
        ).toBe(204)
        expect(await postVisible(scope, conn, 2, ["cull-a"], ["cull-a", "not-on-this-route"])).toBe(
          204,
        )
        const second = await h.segments.next()
        if (second.done || second.value.kind !== "lanes") throw new Error("expected lanes segment")
  ...
  it("an in-flip resolves against its OWN frame's statement, not a later frame's snapshot dip", async () => {
    ...
    await withLiveDrive(
      ...
      async (h) => {
        ...
        // with a's lane.
        expect(await postVisible(scope, conn, 3, ["cull-a"], ["cull-a", "cull-b"])).toBe(204)
        const laneA = await nextLane(laneIter)
        expect(laneA.partonId).toBe("cull-a")
  ...
  it("a later out-flip statement wins over the same id's earlier pending in-flip", async () => {
    ...
    await withLiveDrive(
      ...
      async (h) => {
        ...
        // envelope anyway).
        expect(reportConnectionVisibility(conn, 1, ["cull-b"], ["cull-a", "cull-b"])).toBe(true)
        expect(reportConnectionVisibility(conn, 2, ["cull-b"], ["cull-a"])).toBe(true)
        refreshSelector("cull-a")
  ...
  it("a deferred in-flip is cancelled by a later frame's explicit out-flip", async () => {
    ...
    await withLiveDrive(
      ...
      async (h) => {
        ...
        expect(await postVisible(scope, conn, 3, ["cull-parent"], ["cull-parent"])).toBe(204)
        const second = await h.segments.next()
        if (second.done || second.value.kind !== "lanes") throw new Error("expected lanes segment")
        const laneIter = second.value.lanes[Symbol.asyncIterator]()
        const parentLane = await nextLane(laneIter)
  ...
  it("an envelope for a connection that was never opened answers 404", async () => {
    expect(await postVisible(undefined, "never-opened", 1, ["x"], ["x"])).toBe(404)
  })
  ...
  it("a stale envelope (older seq) cannot regress the set, but its flips still queue", async () => {
    ...
    try {
      expect(await postVisible(undefined, "conn-vis-seq", 2, ["x"], ["x"])).toBe(204)
      // Older seq arrives late: the set stays at seq 2's value…
      expect(await postVisible(undefined, "conn-vis-seq", 1, ["y"], [])).toBe(204)
  ...
  it("a stale late envelope cannot overwrite a newer pending statement about the same id", async () => {
    ...
    try {
      ...
      expect(await postVisible(undefined, "conn-vis-stmt", 2, ["x"], ["x"])).toBe(204)
      expect(await postVisible(undefined, "conn-vis-stmt", 1, ["x"], [])).toBe(204)
      const flips = takeConnectionFlips(session)
      expect(flips.get("x")).toMatchObject({ inView: true, seq: 2 })
    } finally {
  ...
  it("a bump touching a PARKED parton doesn't lane; the flip-in re-renders it fresh", async () => {
    ...
    await withLiveDrive(
      ...
      async (h) => {
        ...
        expect(await postVisible(scope, conn, 2, ["cull-b"], ["cull-a", "cull-b"])).toBe(204)
        const laneIn = await nextLane(laneIter)
        expect(laneIn.partonId).toBe("cull-b")
  ...
  it("a parked parton has no wake surface: its expires() deadline neither lanes nor hot-spins", async () => {
    ...
    await withLiveDrive(
      ...
      async (h) => {
        ...
        if (first.done || first.value.kind !== "payload")
          throw new Error("expected payload segment 0")
        expect(await drainPayloadSegment(first.value)).toContain("pulse:full:1")
        conn = h.connectionId() ?? ""
        expect(conn).not.toBe("")
        ...
        expect(await postVisible(scope, conn, 2, ["cull-pulse"], ["cull-pulse"])).toBe(204)
        const laneIn = await nextLane(laneIter)
