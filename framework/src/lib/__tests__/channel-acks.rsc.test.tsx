...
const ackEnvelope = (connection: string, seq: number, delivered: number): ChannelEnvelope => ({
  connection,
  seq,
...
describe("delivery seqs on the wire", () => {
  it("segments and lanes carry monotonic per-connection seqs; a lane's precedes its muxend", async () => {
    ...
    await withLiveDrive("http://localhost/seq?live=1", PageAB, scope, async (h) => {
      const seqEntries = (): string[] => h.entries.filter((e) => e.tag === "seq").map((e) => e.body)
...
describe("the applied marker + retransmit idempotence", () => {
  it("announces the applied watermark once per advance; a duplicate envelope converges via fp-skip", async () => {
    ...
    await withLiveDrive("http://localhost/applied?live=1", PageAB, scope, async (h) => {
      ...
      const flip: ChannelEnvelope = {
        connection: conn,
        seq: 5,
        frames: [{ kind: "visible", changed: ["ack-b"], visible: ["ack-b"] }],
      }
...
describe("the unacked delivery window", () => {
  it("gates lane opening while exceeded; freed by an ack, dirty ids render their latest state once", async () => {
    ...
    await withLiveDrive("http://localhost/window?live=1", PageWin, scope, async (h) => {
      const first = await h.segments.next()
      if (first.done || first.value.kind !== "payload")
        throw new Error("expected payload segment 0")
      await drainPayloadSegment(first.value)
...
describe("the window vs the client's lazy ack cadence", () => {
  it("a threshold-cadence client never engages the window under sustained lane traffic", async () => {
    ...
    await withLiveDrive("http://localhost/cadence?live=1", PageCad, scope, async (h) => {
      const first = await h.segments.next()
      if (first.done || first.value.kind !== "payload")
        throw new Error("expected payload segment 0")
      await drainPayloadSegment(first.value)
      ...
      for (let i = 0; i < 12; i++) {
        ...
        if (watermark - acked >= 4) {
          envSeq += 1
          expect(await post(scope, ackEnvelope(conn, envSeq, watermark))).toBe(204)
          acked = watermark
        }
      }
  ...
  it("lanes flow up to the window with no acks; past it they coalesce until one cumulative ack frees everything", async () => {
    ...
    await withLiveDrive("http://localhost/silent?live=1", PageSil, scope, async (h) => {
      const first = await h.segments.next()
      if (first.done || first.value.kind !== "payload")
        throw new Error("expected payload segment 0")
      await drainPayloadSegment(first.value)
...
describe("never-acked degrade", () => {
  it("closes after the first-ack deadline with the reason on the session", async () => {
    ...
    await withLiveDrive("http://localhost/degrade?live=1", PageDeg, scope, async (h) => {
      const first = await h.segments.next()
      if (first.done || first.value.kind !== "payload")
        throw new Error("expected payload segment 0")
      await drainPayloadSegment(first.value)
  ...
  it("an acking client is NEVER degraded — any ack frame is the duplex proof", async () => {
    ...
    await withLiveDrive("http://localhost/no-degrade?live=1", PageDeg, scope, async (h) => {
      const first = await h.segments.next()
      if (first.done || first.value.kind !== "payload")
        throw new Error("expected payload segment 0")
      await drainPayloadSegment(first.value)
...
describe("the whole-tree reconcile", () => {
  it("past the cadence, the next wake emits a full payload segment and reopens the lanes region", async () => {
    ...
    await withLiveDrive("http://localhost/reconcile?live=1", PageRec, scope, async (h) => {
      const first = await h.segments.next()
      if (first.done || first.value.kind !== "payload")
        throw new Error("expected payload segment 0")
      ...
      expect(h.entries.filter((e) => e.tag === "seq").map((e) => e.body)).toEqual(["1 0", "2 0"])
...
describe("mirror layering", () => {
  it("a flip statement's cached tokens supersede the ACKED layer — no phantom confirms", async () => {
    ...
    await withLiveDrive("http://localhost/layering?live=1", PageLay, scope, async (h) => {
      const first = await h.segments.next()
      if (first.done || first.value.kind !== "payload")
        throw new Error("expected payload segment 0")
      await drainPayloadSegment(first.value)
      ...
      expect(await post(scope, ackEnvelope(conn, 1, 1))).toBe(204)
      await until(() => (session?.ackedFps.get("lay-a")?.size ?? 0) > 0, "the acked fold")
  ...
  it("the verdict falls back to the acked layer on an optimistic miss", async () => {
    ...
    const render = async (ackedFps: Map<string, Set<string>> | null): Promise<string> => {
    ...
    const fpMatch = cold.match(/"partialId":"iso-a","partialFingerprint":"([^"]+)"/)
